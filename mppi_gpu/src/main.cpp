...
#include <ros/ros.h>
#include <uuv_auv_control_allocator/AUVCommand.h>
...
int main (int argc, char **argv) {
    ...
    node_handle.param<std::string> (name_of_node_ + "/config_file", config_file, "file_not_set");
    node_handle.param<std::string> (name_of_node_ + "/mujoco_key_file", mjkey_file, "file_not_set");
    node_handle.param<std::string> (name_of_node_ + "/out_step_file", out_step_file, "file_not_set");
    node_handle.param<std::string> (name_of_node_ + "/out_traj_file", out_traj_file, "file_not_set");
    node_handle.param<std::string> (name_of_node_ + "/model_file", model_file, "file_not_set");
    node_handle.param<bool> (name_of_node_ + "/use_thruster_manager", use_thruster_manager, "false");
    std::cout << "config file: " << config_file << std::endl;

    // ros::Subscriber state_sub = node_handle.subscribe("TODO", 10, stateCallback);
    ...
    while(!done){
        ...
        geometry_msgs::WrenchStamped command;
        // command.header =
        // command.wrench.force = 
        // command.wrench.torque =

        if(use_thruster_manager){
            uuv_auv_control_allocator::AUVCommand msg;
            // Add data to message
            // msg.header = 
...
// void stateCallback(const TODO::TODOmsgtype& msg){
//     msg.q_pos
//     msg.q_vel;
// }
...
void parse_config (std::string& config_file,
    ...
    //     exit(1);
    // }
    // model_file = config["env"].as<std::string>(); ***Now done as ros param***
